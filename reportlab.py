...
@app.post("/repartidor/cargar_firma_factura")
def upload(factura_id: int = Form(1), file: UploadFile = File(...), db: Session = Depends(get_session)):
    try:
        factura = db.query(Factura).get(factura_id)
        url_firma = f'{factura.serie}-{factura.numero_factura}.png'
        factura.firma = url_firma


        contents = file.file.read()
 
        ruta2 = f"./static/firmas/{url_firma}"
    ...
    return {"message": f"Successfully uploaded {url_firma}"}

class Dato_pdf(BaseModel):
...
#---------------------------------------------------------------
# IMPRIMIR EN PDF 
@app.post('/repartidor/imprimir_pdf')
def imprimir_pdf( dato:Dato_pdf , db: Session = Depends(get_session)):
    ...
    p.drawImage('static/imagenes/1.png',30,10,555,820,preserveAspectRatio=False,mask='auto')
    ...
    # NIF DEL CLIENTE
    p.setFont('Helvetica-Bold',10)
    p.drawString(20*mm,212*mm,f'{factura.nif_contacto}') 
    ...
    ruta = f'facturas/prueba2.pdf'
    ruta2 = f'./static/facturas/{factura.serie}-{factura.numero_factura}.pdf'
    ...
    #headers = {'Content-Disposition': 'inline; filename="prueba.pdf"'}
